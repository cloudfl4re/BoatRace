...
在 `messages.yml` 增加页面打开、无权限、无活动比赛、暂停成功、继续成功、结束确认、结束成功、首名烟花、倒计时 Title 主标题/副标题、发车 Title 主标题/副标题、实时第一名 ActionBar、无冲线记录提示、编辑保存、编辑回滚、无效 action 和配置错误消息；所有玩家文本只从消息文件读取。
