...
为 BoatRace 增加一个通过 `/race gui` 打开的可配置箱子 GUI，覆盖现有比赛、自由计时、赛道管理和排行榜功能；管理员可以通过 `/race gui view <菜单名>` 拖动整理菜单布局，关闭编辑视图时自动保存槽位配置。
...
- `main.yml`：创建、加入、开始、退出、取消、状态、最近比赛、排行榜和管理员入口。
- `race-control.yml`：当前房间、参赛者、暂停、继续、结束、取消；仅当前比赛成员查看，控制按钮需要 `boatrace.admin`。
- `track-list.yml`：赛道列表、赛道信息、创建、编辑、删除入口。
- `track-editor.yml`：位置 1/2、起点、记录点、发车位、预览、保存、取消。
- `confirm.yml`：结束比赛、删除赛道等不可逆操作的确认页。

菜单采用 PlayerMenu 风格的槽位配置，但使用 BoatRace 自己的解析器：

```yaml
title: "<dark_blue><bold>BoatRace</bold></dark_blue>"
size: 54
